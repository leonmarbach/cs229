...
class QP:
    ...
    def train_GD(self, X, Y, eta = 8e-2, max_step = 1000, 
        ...
        for t in range(max_step):
            ...
            self.theta -= eta * gradient_theta
            self.phi -= eta * gradient_phi
    ...
    def gradient(self, X, Y):
        ...
        n = X.shape[0]
        grad_theta = np.zeros(self.dim)
        grad_phi = np.zeros(self.dim)
        for i in range(n):
            inner_product = np.dot(X[i].T, self.theta ** 2 - self.phi ** 2)
            grad_theta += (1/n) * (inner_product - Y[i]) * (self.theta * X[i])
            grad_phi -= (1/n) * (inner_product - Y[i]) * (self.phi * X[i])
