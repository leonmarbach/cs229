...
def linear_model_main():
    ...
    beta_1 = X.T.dot(np.linalg.inv(X.dot(X.T))).dot(Y) + 2 * ns[0]
    beta_2 = X.T.dot(np.linalg.inv(X.dot(X.T))).dot(Y) + 3 * ns[1]
    beta_3 = X.T.dot(np.linalg.inv(X.dot(X.T))).dot(Y) + 4 * ns[2]

    generate_plot([beta_0, beta_1, beta_2, beta_3], X, Y, X_val, Y_val, save_path_linear)
