...
def forward_prop(data, one_hot_labels, params):
    ...
    hidden_layer_activations = sigmoid(np.dot(data, W1) + b1)

    output_layer_activations = softmax(np.dot(hidden_layer_activations, W2) + b2)

    average_ce_loss = (- 1 / data.shape[0]) * np.sum(one_hot_labels * np.log(output_layer_activations))

    return hidden_layer_activations, output_layer_activations, average_ce_loss
...
def backward_prop(data, one_hot_labels, params, forward_prop_func):
    ...
    return backward_prop_regularized(data, one_hot_labels, params, forward_prop_func, 0)
...
def backward_prop_regularized(data, one_hot_labels, params, forward_prop_func, reg):
    ...
    hidden_activations, output_activations, loss = forward_prop_func(data, one_hot_labels, params)

    grad_output = output_activations - one_hot_labels
    grad_W2 = (1 / data.shape[0]) * hidden_activations.T.dot(grad_output) + reg * 2 * W2
    grad_b2 = (1 / data.shape[0]) * np.sum(grad_output, axis=0)

    grad_hidden = np.dot(grad_output, W2.T) * hidden_activations * (1 - hidden_activations)
    grad_W1 = (1 / data.shape[0]) * np.dot(data.T, grad_hidden) + reg * 2 * W1
    grad_b1 = (1 / data.shape[0]) * np.sum(grad_hidden, axis=0)
...
def gradient_descent_epoch(train_data, one_hot_train_labels, learning_rate, batch_size, params, forward_prop_func, backward_prop_func):
    ...
    for i in range(train_data.shape[0] // batch_size):
        ...
        batch_data = train_data[begin_index:end_index]
        batch_labels = one_hot_train_labels[begin_index:end_index]
