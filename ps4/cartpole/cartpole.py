...
def choose_action(state, mdp_data):
    ...
    transition_probs = mdp_data['transition_probs'][state, :, :]

    if np.dot(transition_probs[:, 0], mdp_data['value']) > np.dot(transition_probs[:, 1], mdp_data['value']):
        action = 0
...
def update_mdp_transition_probs_reward(mdp_data):
    ...
    for action in [0, 1]:
        for state in range(mdp_data['num_states']):
            if np.sum(mdp_data['transition_counts'][state, :, action]) > 0:
                mdp_data['transition_probs'][state, :, action] = mdp_data['transition_counts'][state, :, action] / np.sum(mdp_data['transition_counts'][state, :, action])
